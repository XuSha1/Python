...
g1=df.groupBy("captcha_id",F.substring("request_time",1,19).alias("time")).count().withColumnRenamed("count","gps").filter(df.captcha_id!='')
pandas_df=g1.toPandas()#转换成pandas的dataframe
data_pivot=pandas_df.pivot_table(index=["captcha_id"],columns=["time"],values=["gps"])
