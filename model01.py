...
def RPO_LR(X_train,y_train):
    ...
    print("RandomizedSearchCV of LR took %.2f second"%((time()-start)))
...
def RPO_RF(X_train,y_train):
   ...
   #     "pca__n_components":sp_randint(1,16),
    ...
    print("RandomizedSearchCV of RF took %.2f second" % ((time() - start)))
...
def RPO_XGBoost(X_train,y_train):
    ...
    start=time()
    random_search.fit(X_train,y_train)
    print("RandomizedSearchCV of XGBoost took %.2f second" % ((time() - start)))
    report(random_search.cv_results_)
...
def OneClassSvm(X_train,y_train):
    clf = svm.OneClassSVM(nu=0.1, kernel='rbf', gamma=0.1)
    start = time.time()
    clf.fit(X_train)
    print('OneClassSVM训练时间为%.2f s' % (time.time() - start))
...
if __name__=='__main__':
    ...
    y1 = pd.DataFrame(np.vstack((np.ones((355808, 1)),
                                -np.ones((375600, 1)))), columns=['label'])  # 0标记为人，1标记为机器
    X=np.array(X)
    ...
    X_train01, X_test01, y_train01, y_test01 = train_test_split(X, y1,
                                                        test_size=0.20, random_state=1)
    ###测试数据
    ...
    #OneClassSvm(X_train01,y_train01)
